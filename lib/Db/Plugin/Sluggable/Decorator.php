...
class Db_Plugin_Sluggable_Decorator extends Db_Decorator
{
	...
	public function getFromObjectToDataCode($object_var, $dest_var, $is_update = false)
	{
		...
		{
			return 'Rdb::isChanged('.$object_var.', \''.$this->source->getProperty().'\') && '.$dest_var.'[\''.$this->getColumn().'\'] = '.$this->callable.'('.$this->source->getCastFromPhpCode($this->source->getFromObjectCode($object_var)).');';
		}
		else
		{
			return '';
